...
class SavingHander():
    ...
    def export(self):
        filename, _ = QFileDialog.getSaveFileName(self.app.ui.centralwidget, "Notensatz exportieren",
                                                  self.generate_filename() + ".pdf", "*.pdf")
        if (filename):
    ...
    def save_as(self):
        filename, _ = QFileDialog.getSaveFileName(self.app.ui.centralwidget, "Notensatz speichern",
                                                  self.generate_filename() + "." + Settings.Document.FILE_EXTENSION,
                                                  "*." + Settings.Document.FILE_EXTENSION)
        if (filename):
    ...
    def open_file(self):
        filename, _ = QFileDialog.getOpenFileName(self.app.ui.centralwidget, "Notensatz öffnen", "",
                                                  "*." + Settings.Document.FILE_EXTENSION)
        if filename:
            self.app.current_file_name = filename
            self.app.current_file_saved = True
    ...
    def handle_close_event(self, app: App, action):
        if (not self.app.ui.in_welcome_screen):
            close_box = QMessageBox(QMessageBox.Question, "Notensatz", "Wollen Sie das aktuelle Dokument speichern?",
                                    QMessageBox.Save | QMessageBox.Discard | QMessageBox.Cancel, app.window)
