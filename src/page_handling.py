...
class PageHandler():
    ...
    def create_empty_page(self, page_number: int, new_first_page=False, heading_text="", sub_heading_text="",
                          composer_text="", tempo_text=""):
        ...
        if (new_first_page):
            ...
            heading = DocumentTextitem(True, heading_text, real_font_size(20), Settings.Layout.MARGIN, "center",
                                       Settings.Layout.MARGIN, True)
            sub_heading = DocumentTextitem(True, sub_heading_text, real_font_size(12),
                                           heading.qt().y() + heading.qt().sceneBoundingRect().height(), "center",
                                           Settings.Layout.MARGIN, False)
            composer = DocumentTextitem(True, composer_text, real_font_size(12),
                                        sub_heading.qt().y() + sub_heading.qt().sceneBoundingRect().height(), "right",
                                        Settings.Layout.MARGIN, True)
            tempo = DocumentTextitem(True, tempo_text, real_font_size(12),
                                     composer.qt().y() + composer.qt().sceneBoundingRect().height(), "left",
                                     2 * Settings.Layout.MARGIN, True)
    ...
    def update_page_info_and_button_text(self):
        self.app.ui.current_page_label.setText(
            "Seite " + str(self.app.current_page + 1) + " von " + str(len(self.app.document_ui.pages)))
    ...
    def delete_last_system(self):
        if (len(self.app.document_ui.systems) < 2):
            info_box = QMessageBox(QMessageBox.Information, "Information", "Sie können das erste System nicht löschen.",
                                   QMessageBox.Yes, self.app.window)
        ...
        if (self.app.show_warning_box):
            warning_box = QMessageBox(QMessageBox.Information, "System Löschen",
                                      "Wollen Sie das letzte System wirklich löschen?",
                                      QMessageBox.Yes | QMessageBox.No, self.app.window)
