from PyQt5.QtWidgets import QGraphicsBlurEffect, QLabel, QPushButton, QMenu, QHBoxLayout, QApplication, QMainWindow, \
    QDialog, QWidget
...
class NewDocumentDialogUI(Ui_NewDocumentDialog):
    def setupUi(self, NewDocumentDialog):
        ...
        self.voices_combo_boxes = [self.voice_1_combo_box, self.voice_2_combo_box, self.voice_3_combo_box,
                                   self.voice_4_combo_box]
...
class App(QApplication):
    def __init__(self):
        ...
        # set icons
        icon = QIcon()
        icon.addPixmap(QPixmap("assets/icon.png"), QIcon.Normal, QIcon.Off)
    ...
    def window_resize(self, event):
        if (self.ui.in_welcome_screen):
            self.ui.welcome_label.move((self.window.width() - self.ui.welcome_label.width()) / 2,
                                       (self.window.height() - self.ui.welcome_label.height()) / 2)
            self.ui.welcome_button_new.move((
                                                        self.window.width() - self.ui.welcome_button_new.width()) / 2 + self.ui.welcome_button_new.width() / 2 + 10,
                                            self.ui.welcome_label.y() + self.ui.welcome_label.height() + 20)
            self.ui.welcome_button_open.move((
                                                         self.window.width() - self.ui.welcome_button_open.width()) / 2 - self.ui.welcome_button_open.width() / 2 - 10,
                                             self.ui.welcome_label.y() + self.ui.welcome_label.height() + 20)
