...
class Musicitem(N_QGraphicsTextItem):
            ...
            if (len(self.key) > 4):
                ...
                if (self.key[2] == "timeSigCombNumerator"):
                    possible_widths.append(
                        Musicitem.get_character_width(self.key[1]) + Musicitem.get_character_width(self.key[3]))
                    if (len(self.key) > 6 and self.key[6] == "timeSigCombDenominator"):
                        possible_widths.append(
                            Musicitem.get_character_width(self.key[5]) + Musicitem.get_character_width(self.key[7]))
                elif (self.key[4] == "timeSigCombDenominator"):
                    possible_widths.append(
                        Musicitem.get_character_width(self.key[3]) + Musicitem.get_character_width(self.key[5]))
            else:
    ...
    def get_bounding_rect(self) -> QRectF:
        if (type(self.key) != str):
            ...
            return QRectF(
                self.get_real_x(),
                self.get_real_y() - Musicitem.spec_to_px(
                    get_specification("glyphBBoxes", self.key)["bBoxNE"][1]) * self.qt().transform().m22(),
                self.get_real_width(),
...
class TimeSignature(Musicitem):
    ...
    def gen_unicode_combi(self) -> List[str]:
        ...
        if (self.fundamental_beats > 9):
            combi_list.extend(
                ["timeSigCombNumerator", "timeSig" + str(self.fundamental_beats // 10), "timeSigCombNumerator",
                 "timeSig" + str(self.fundamental_beats % 10)])
        else:
            combi_list.extend(["timeSigCombNumerator", "timeSig" + str(self.fundamental_beats)])

        if (self.note_value > 9):
            combi_list.extend(
                ["timeSigCombDenominator", "timeSig" + str(self.note_value // 10), "timeSigCombDenominator",
                 "timeSig" + str(self.note_value % 10)])
        else:
...
class Bar(N_QGraphicsItemGroup):
    ...
    def find_places(self, potential_items: List[Musicitem], next_bar_x: float) -> List[List[float]]:
        ...
        if (len(self.objects) >= 1):
            first_end_x = self.objects[
                              0].get_real_relative_x() + self.qt().scenePos().x() - items_width - Musicitem.MIN_OBJ_DIST
            ...
            for n, obj in enumerate(self.objects):
                ...
                if (n < len(self.objects) - 1):
                    end_x = self.objects[
                                n + 1].get_real_relative_x() + self.qt().scenePos().x() - items_width - Musicitem.MIN_OBJ_DIST
...
class Stave(N_QGraphicsItemGroup):
    ...
    def __init__(self, clef_key: str, width: float, key_signature: KeySignature):
        ...
        for i in range(self.key_signature.number):
            ...
            if (i > 0):
                x = self.key_signature_accidentals[i - 1].get_real_relative_x() + self.key_signature_accidentals[
                    i - 1].get_real_width()
            y = Musicitem.get_line_y(
                KeySignature.accidentals_positions[self.key_signature.type - 1][i] + Stave.clefs[clef_key][
                    "accidental_shift"])
    ...
    def split_bar(self, bar_index: int):
        ...
        for n, obj in enumerate(self.bars[bar_index].objects):
            if (obj.get_real_relative_x() + self.bars[bar_index].qt().scenePos().x() > self.bars[
                bar_index + 1].qt().scenePos().x()):
...
class System(N_QGraphicsItemGroup):
    ...
    def __init__(self, page_index: int, voices: int, pos: QPointF, clefs_tabel: List[str], key_signature: KeySignature,
                 with_piano: bool, time_signature: TimeSignature, first_system: bool = False):
        super().__init__(QGraphicsItemGroup())
        ...
        self.left_bar_line.qt().setTransform(
            QTransform().scale(1, (self.get_bottom_y() - self.qt().y()) / Musicitem.EM))
        self.qt().addToGroup(self.left_bar_line.qt())
        ...
        self.right_bar_line.qt().setTransform(
            QTransform().scale(1, (self.get_bottom_y() - self.qt().y()) / Musicitem.EM))
    ...
    def addbrace(self):
        ...
        self.brace.set_real_pos(-self.brace.get_real_width() - Musicitem.EM / 16,
                                self.staves[-1].qt().y() + Musicitem.EM)

    def get_start_x(self):
        staves_start_x = []
        for stave in self.staves:
            if (self.key_signature.number > 0):
                staves_start_x.append(stave.key_signature_accidentals[-1].qt().sceneBoundingRect().width() +
                                      stave.key_signature_accidentals[-1].qt().pos().x())
            else:
    ...
    def set_normal_end_bar_line(self):
        ...
        self.right_bar_line.set_real_pos(self.width - self.right_bar_line.get_real_width(),
                                         self.get_bottom_y() - self.qt().y())
