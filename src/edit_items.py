...
class Page(N_QGraphicsScene):
    def __init__(self, page_number: int):
        ...
        if (page_number > 1):
            ...
            if (page_number % 2 == 0):
                align = "left"
            self.page_number_text = DocumentTextitem(False, str(page_number), real_font_size(10),
                                                     Settings.Layout.MARGIN / 2, align, Settings.Layout.MARGIN, False)
...
class DocumentTextitem(N_QGraphicsTextItem):
    def __init__(self, allow_interaction: bool, text: str, fontSize: float, y: float, alignment: str,
                 align_spacing: float, bold: bool):
        """
    ...
    def crop_text(self):
        ...
        while (self.qt().boundingRect().width() > (Settings.Layout.WIDTH - 2 * self.align_spacing)):
            ...
            self.qt().setPlainText(
                self.qt().toPlainText()[:old_cursor_pos - 1] + self.qt().toPlainText()[old_cursor_pos:])
